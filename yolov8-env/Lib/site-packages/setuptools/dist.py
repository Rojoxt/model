...
def check_nsp(dist, attr, value):
    """Verify that namespace packages are val"""
    ns_packages = value
...
def check_extras(dist, attr, value):
    """Verify that extras_require mapping is val"""
    try:
        list(itertools.starmap(_check_extra, value.items()))
    except (TypeError, ValueError, AttributeError) as e:
        raise DistutilsSetupError(
            "'extras_require' must be a dictionary whose values are "
            "strings or lists of strings containing val project/version "
...
def check_requirements(dist, attr, value):
    """Verify that install_requires is a val requirements list"""
    try:
        ...
        tmpl = (
            ...
            "containing val project/version requirement specifiers; {error}"
        )
        raise DistutilsSetupError(
...
def check_specifier(dist, attr, value):
    """Verify that value is a val version specifier"""
    try:
        packaging.specifiers.SpecifierSet(value)
    except (packaging.specifiers.InvalidSpecifier, AttributeError) as error:
        tmpl = (
            "{attr!r} must be a string "
            "containing val version specifiers; {error}"
        )
        raise DistutilsSetupError(
            tmpl.format(attr=attr, error=error)
        ) from error
...
def check_packages(dist, attr, value):
    for pkgname in value:
        if not re.match(r'\w+(\.\w+)*', pkgname):
            distutils.log.warn(
                "WARNING: %r not a val package name; please use only "
